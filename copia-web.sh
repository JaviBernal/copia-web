...
# 0.3.2 160627 solucionado bug en el grep del wp-config.php si habia otra linea DB_NAME comentada
...
then   # Si es un wordpress averiguar el nombre de la BD del wp-config.php
   ...
   then
      ...
      DBNAME=`grep '^define' $WEBROOT/$1/wp-config.php |grep DB_NAME|cut -d "'" -f 4`
   else
